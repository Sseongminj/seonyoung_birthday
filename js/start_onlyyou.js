...
function init_onlyyou() {
  // 두 div의 높이를 초기화
  // $("#div_onlyyou").css({ "min-height": $(window).height() + 300 + "px" });
  // $("#div_oy_inner").css({ "min-height": $(window).height() + 300 + "px" });
  $("#div_onlyyou").css({ "min-height": $(window).height() + "px" });
  $("#div_oy_inner").css({ "min-height": $(window).height() + "px" });
...
function oy_show_benefit() {
  ...
  if (index_text_oy < count_text_oy) {
    ...
    if ($(document).height() - oy_text_height < 520) {
      ...
      // $("#div_onlyyou").css({ "min-height": $(document).height() + 400 + "px" });
      // $("#div_oy_inner").css({ "min-height": $(document).height() + 100 + "px" });
      $("#div_onlyyou").css({ "min-height": $(document).height() + "px" });
      $("#div_oy_inner").css({ "min-height": $(document).height() + "px" });
      console.log("update the document height + 200");
    }
    index_text_oy++;
  } else {
    oy_show_note();
